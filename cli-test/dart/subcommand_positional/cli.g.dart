...
class CLI_Sub {
  Func<CLI_Sub_Input>? FUNC;
  
}


typedef CLI_Sub_Input = ({
...
CLI_Sub_Input _resolve_CLI_Sub_Input(List<String> restArgs) {
...
void run(CLI cli, List<String> args) {
  ...
  case "sub":
    final funcMethod = cli.sub.FUNC;
    if (funcMethod == null) {
      throw Exception("'${ "sub" }' is unsupported: cli.sub.FUNC not assigned");
    }

    CLI_Sub_Input? input;
    Exception? err;
    try {
      input = _resolve_CLI_Sub_Input(restArgs);
    } on Exception catch (e) {
      err = e;
    }
    funcMethod(subcommandPath, input, err);
