...
import (
	_ "embed"
	"fmt"
	"github.com/Jumpaku/cyamli/dart"
...
//go:generate go run github.com/Jumpaku/cyamli@latest generate golang -schema-path=cli.yaml -out-path=cli.gen.go -package=cyamli
...
func Execute(args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) (code int) {
	...
	cli.Generate.Dart.FUNC = impl.generateDart
...
func (i implementation) generateDart(subcommand []string, input CLI_GenerateDart_Input, inputErr error) (err error) {
	panicIfErrorf(inputErr, "failed to parse arguments ('cyamli %s -help' to see help)", strings.Join(subcommand, " "))
	...
	err = dart.Generate(s, info.Name, info.Version, writer)
	panicIfErrorf(err, "fail to generate cli in Dart")
